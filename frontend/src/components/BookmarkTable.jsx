...
const BookmarkTable = () => {
  ...
  useEffect(() => {
    ...
    const savedBookmarkCategories = localStorage.getItem(
      "bookmarkCategoryAssignments",
    );
  ...
  useEffect(() => {
    ...
    setFilteredBookmarks(
      bookmarkedJobs.filter(
        (bookmark) => bookmarkCategories[bookmark._id] === selectedCategory,
      ),
    );
  ...
  const applyForJob = async (jobId, bookmarkId) => {
    try {
      ...
      if (applyResponse.data.success) {
        ...
        toast.error(
          applyResponse.data.message || "Failed to apply for job",
        );
      }
    } catch (error) {
  ...
  const removeBookmark = async (jobId, bookmarkId) => {
    try {
      ...
      const response = await axios.post(
        `${BOOKMARK_API_END_POINT}/get/${jobId}`,
        {},
        { withCredentials: true },
      );
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger asChild>
            <Button
              variant="outline"
              className="flex items-center gap-1 border-[#7209b7] text-[#7209b7] hover:bg-[#7209b7]/20 hover:text-[#7209b7]"
            >
                ...
                <TableRow key={bookmark._id}>
                  ...
                  <TableCell>
                    <Select
                      ...
                      onValueChange={(value) =>
                        assignCategory(bookmark._id, value)
                      }
                    >
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      <Button
                        ...
                        onClick={() =>
                          applyForJob(bookmark.job?._id, bookmark._id)
                        }
                        disabled={applyingJobs[bookmark.job?._id]}
                      >
                        {applyingJobs[bookmark.job?._id]
                          ? "Applying..."
                          : "Apply"}
                      </Button>
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          removeBookmark(bookmark.job?._id, bookmark._id)
                        }
